...
|---|---|---|---|
| M3.0 | Common hypothesis/candidate adapter, source distinctions, cross-batch deduplication, existing kernel invocation and resumable lineage | Real paper migration plus rejected/failed/survivor engineering paths; persisted restart and duplicate checks | Partial: paper rejection and numerical survivor core verified; cross-batch campaign integration pending |
| M3.1 | Budgeted LLM generation and manual/paper imports through one entry | Real configured model request, structured-response validation, exhausted budget/error tests, at least one bounded research batch | Pending |
| M3.2 | Research-only feedback/refinement, fixed campaign budget and immutable children | Real multi-round execution and protected-feedback rejection; all failed proposals retained and multiple-testing family tracked across rounds | Pending |
| M3.3 | Searchable trajectory memory, parent-linked mutation/crossover, deterministic resume | Restarted run with recorded lineage; retrieval excluding protected outcomes; lineage and duplicate validation | Pending |
| M3.4 | AlphaForge and AlphaSAGE external generator adapters | Verified upstream interfaces/revisions; genuine produced asset imported, unsupported DSL rejected explicitly; no substitute algorithm labelled as upstream | Pending |
...
| M3.6 | Multi-source data/candidate boundary | Daily formula inputs and existing PIT fundamental panels through one explicit data contract; field availability/identity tests; unknown event/text/microstructure fields fail rather than fabricated | Pending |
