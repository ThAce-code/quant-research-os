...
**M2整体仍未完成：历史盈利/成长采集被BaoStock明确拒绝，错误10001011“黑名单用户，请与管理员联系”。** BaoStock请求已停止并保全缓存；2026-09-06新增AKShare/东方财富替代源小样本验证成功，尚未完成整批迁移，不能删掉剩余数据任务。有效滚动模型已完成并得出NO_GO，没有独立Alpha、KEEP池为0。

## AKShare替代路线更新

用户已授权尝试独立替代源。两只股票返回107期历史指标；按原口径从报表重建五字段，17个有缓存可比值全部吻合至1e-6内，另3个缺失源字段也已取到。8次HTTP请求，没有请求BaoStock。仍需扩大历史覆盖、核对公告日期和修订、建立独立来源面板；不能冒充BaoStock响应或宣称全量验收。详见[AKShare验证](AKSHARE_FEASIBILITY.md)。
...
1. 对已成功取数的AKShare替代路线冻结映射、日期与来源规则，扩大覆盖及跨源核验；通过后按原股票/季度/字段范围分批补齐独立面板。BaoStock原40,520请求计划及8,354缺失响应保留，不伪造响应完成数。
...
回归：`154 passed, 5 warnings in 21.47s`。M0数据身份未变。公开报告是小型精确副本，183份来源文件已核对哈希；原始数据、完整面板、模型、SQLite留在本地。来源run和SHA256见[sources.json](results/sources.json)。
