...
状态核对日期：2026-09-06（最新工程run UTC为2026-09-07）。本文件是唯一当前状态入口，CONTEXT.md只保留指针；旧聊天和旧失败run不代表当前状态。
...
这里的完成指已约定的有限研究和条件决策树，不代表穷尽所有因子族、取得全市场数据或找到可交易Alpha。M0冻结、M1关闭扩展。M3.0日频公式接入链已完成真实合格候选的全规模验证，Alpha#34模型结论仍是NO_GO；M3.4外部生成器已完成有界真实训练、导入与初筛验收。M3.1–M3.3及M3.6部分实现，M3.5未完成，M3整体尚未完成。当前完整状态见[M3验收路线](M3_ROADMAP.md)及本文末尾最新批次；下方首批和工程fixture均为历史证据。
