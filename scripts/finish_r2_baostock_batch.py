...
串行请求、每项结束后间隔至少1秒。原采集器遇错误停止；修复后的用户续采入口对明确无数据的10001001及只读查询10002007执行有界重新登录，本批累计{result.get('session_refreshes',0)}次，原失败全部归档。登录失效每项最多补发一次，接收失败每项最多两次并退避15/30秒；黑名单、部分数据、未知错误、分页歧义或预算用尽仍停止。成功/失败/未请求项分别保留，不改写旧BaoStock失败记录、巨潮原件或东方财富面板。
