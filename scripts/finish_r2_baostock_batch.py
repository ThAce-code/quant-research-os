...
串行请求、每项结束后间隔至少1秒。原采集器遇错误停止；修复后的用户续采入口只对明确无数据的10001001执行有界重新登录，本批累计{result.get('session_refreshes',0)}次，原失败全部归档。黑名单、通信错误、分页歧义或同项再次失效仍停止。成功/失败/未请求项分别保留，不改写旧BaoStock失败记录、巨潮原件或东方财富面板。
